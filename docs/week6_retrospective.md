...
**메타데이터 필터링·쿼리 리라이팅에 대한 주의:**
- **메타데이터 필터링**은 검색 *전* 카탈로그 전체 제목을 enumerate하는 방식이 아니라(컨텐츠 확장 시 깨짐), **검색된 chunk의 제목/날짜를 grade(5단계)에 주입**해 주제·시점 불일치 chunk를 거르는 방식이다 — 검색 결과 k개만 보므로 **컨텐츠가 늘어도 안전**.
- **쿼리 리라이팅**은 두 갈래: ① **proactive 분해**(2단계 `decompose`, V2 — 질문 분석으로 미리 쪼갬) ② **reactive 재작성**(7단계 `rewrite_query`, V0+ — grade 실패 후 재시도). in-corpus 질문에선 ①만 실제로 발화하고 ②는 OOD 질문에서만 발화(§5 참조).
- **preprocessing(노이즈 제거)** 는 시도했으나 1500자 chunk-level 분류가 0개 drop → 효과 없음(§4).
...
V0의 한계는 "rewrite/retry 루프가 in-corpus 질문에서 한 번도 발화하지 않는다"였다(grade가 늘 통과 → §5 참조). 원인은 분해가 **reactive**(grade 실패 후)였다는 점. V2는 분해를 **proactive**로 옮긴다: 진입부에 `analyze_query`(단답 vs 복합 분류)를 두고, **복합이면 `decompose`로 multi-query를 먼저 만든 뒤 retrieve**한다.
...
**해석:** V2는 **context 완전성(precision·recall 만점)을 위해 비용(latency·LLM 호출)을 가장 많이 쓰는** 구성. "분해가 실제로 발화하게 만든" 것이 V0와의 결정적 차이다. 정확도 우선이면 V2, 비용 민감하면 V0가 sweet spot(아래 §5 결론).
...
## 7. 왜 단일 RAG가 아니라 Agentic 구조인가 (한 단락)
